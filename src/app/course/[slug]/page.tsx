    ...
    <div>
      ...
      <div className="mb-6 border-b border-slate-200 pb-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <span className={`rounded px-2 py-0.5 text-xs font-semibold ${color?.badge ?? "bg-slate-100 text-slate-600"}`}>
            {lesson.meta.subject}
          </span>
      ...
      <LessonViewer
        ...
        prev={
          prev
            ? { slug: prev.slug, title: prev.title, subject: prev.subject }
            : null
        }
        next={
          next
            ? { slug: next.slug, title: next.title, subject: next.subject }
            : null
        }
      />
    </div>
